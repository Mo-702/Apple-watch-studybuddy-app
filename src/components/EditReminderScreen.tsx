...
export function EditReminderScreen({ reminder, onBack, onSave }: EditReminderScreenProps) {
  const [step, setStep] = useState<'edit' | 'success'>('edit');
  const [selectedDate, setSelectedDate] = useState(reminder.date);
  const [selectedTime, setSelectedTime] = useState(reminder.time);
  const [timeRemaining, setTimeRemaining] = useState(calculateTimeRemaining(reminder.date, reminder.time));
  ...
  const handleSaveChanges = () => {
    const updatedReminder: Reminder = {
      ...
      date: selectedDate,
      time: selectedTime,
      daysLeft: timeRemaining.totalDays,
    };
  ...
  const reminderTypeLabel = getReminderTypeLabel(reminder.type);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 pb-3">
        ...
        <h2 className="text-white">
          {step === 'edit' && 'Edit Reminder'}
          {step === 'success' && 'Updated!'}
        </h2>
        {step === 'edit' && (
          <p className="text-gray-400 text-xs">{reminder.courseName}</p>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {step === 'edit' && (
          <>
            ...
            <div className="bg-gray-900 rounded-2xl p-4 mb-4">
              ...
              <div className="text-gray-400 text-xs">{reminderTypeLabel}</div>
            </div>
        ...
        {step === 'success' && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-white mb-2">Reminder Updated Successfully</h3>
